...
IMAGE_NAME=dobri1408/hidrosalt-consulting-backend
IMAGE_TAG=latest
...
.PHONY: test
test: ## run tests
	./bin/zope-testrunner --auto-color --auto-progress --test-path src/hidrosalt_consulting/src/

.PHONY: test_quiet
test_quiet: ## run tests removing deprecation warnings
	PYTHONWARNINGS=ignore ./bin/zope-testrunner --auto-color --auto-progress --test-path src/hidrosalt_consulting/src/
